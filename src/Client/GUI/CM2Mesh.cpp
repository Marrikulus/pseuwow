...
void CM2Mesh::LinkChildMeshes (IAnimatedMeshSceneNode *ParentMeshNode, ISceneManager *smgr)
{
	...
	for(u16 s=0; s < Skins[CM2Mesh::SkinID].Submeshes.size(); s++)
	{
		...
		// Create a node and link it to a child mesh 
		scene::IAnimatedMeshSceneNode* ChildMeshNode = smgr->addAnimatedMeshSceneNode(smgr->getMeshCache()->getMeshByName(Skins[CM2Mesh::SkinID].Submeshes[s].UniqueName.c_str()));
